...
export default function Dashboard() {
  ...
  const uploadPdf = async () => {
    ...
    try {
      ...
      if (res.status === 202) {
        toast.success("PDF Uploaded Started ");
  ...
  const pollDocumentStatus = (documentId: string) => {
    const interval = setInterval(async () => {
      try {
        const res = await api.get(`/document/${documentId}/status/`);
        ...
        if (status == "completed" || status === "failed") {
          ...
          toast.success(`Processing ${status}`);
          fetchPdfs();
        }
      } catch (error) {
        clearInterval(interval);
  ...
  return (
    <div className="dashboard">
      ...
      <div className="dashboard-card">
        ...
        <button onClick={fetchPdfs} className="loadbutton">
          Load Documents
        </button>
          ...
          {pdfs.length > 0 ? (
            pdfs.map((pdf) => (
              <div
                ...
                key={pdf.id}
                onClick={() => navigate(`/document/${pdf.id}`)}
              >
                <div>
                  <strong>{pdf.name}</strong>
                  <p className={`status ${pdf.processing_status}`}>
      ...
      <div className="dashboard-card">
        ...
        <div className="upload-section">
          ...
          <button onClick={uploadYoutube}>Upload URL</button>
        </div>

        <button onClick={fetchUrls} className="loadbutton">
          Load Url's
        </button>
          ...
          {urls.length > 0 ? (
            urls.map((url) => (
              <div className="document-item" key={url.id}>
                ...
                <button
                  className="del"
                  onClick={() => deleteYoutubeUrl(url.id)}
                >
                  <MdDelete />
                </button>
              </div>
